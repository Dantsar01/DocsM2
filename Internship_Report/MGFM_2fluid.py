...
def to_riemann(u, rho, mask, K, gamma):
    """
    Convert (u, rho) → (R+, R-), using per-cell EOS constants selected by
    a fluid mask (0 → fluid 1, 1 → fluid 2).

    Parameters
    ----------
    u, rho : arrays or scalars — state to convert
    mask   : array or scalar, 0/1 (or bool) — 0 selects (K[0], gamma[0]),
             1 selects (K[1], gamma[1]).  Every call site currently passes
             a uniformly-valued mask (each solver is still single-fluid),
             but the formula is fully vectorized per-cell so a genuinely
             mixed mask will work unchanged once callers build one.
    K, gamma : length-2 sequences (K1, K2), (gamma1, gamma2) — EOS
               constants of fluid 1 and fluid 2 respectively.

    Per selected fluid k:
        gamma_k == 1  (linear EOS p = K*rho, c = sqrt(K) = const):
            R± = u ± sqrt(K) * ln(rho)
        gamma_k != 1  (isentropic EOS p = K*rho^gamma):
            R± = u ± 2c/(gamma-1),  c = sound_speed(rho, K, gamma)
    """
    mask    = np.asarray(mask)
    K_loc   = np.where(mask == 0, K[0],     K[1])
    gam_loc = np.where(mask == 0, gamma[0], gamma[1])

    is_linear = (gam_loc == 1.0)
    c = sound_speed(rho, K_loc, gam_loc)
    with np.errstate(divide='ignore', invalid='ignore'):
        dR_gen = 2.0 * c / (gam_loc - 1.0)
    dR_lin = np.sqrt(K_loc) * np.log(rho)
    dR = np.where(is_linear, dR_lin, dR_gen)

    return u + dR, u - dR


def from_riemann(R_plus, R_minus, mask, K, gamma):
    """
    Convert (R+, R-) → (u, rho, c), using per-cell EOS constants selected
    by a fluid mask (0 → fluid 1, 1 → fluid 2).  See to_riemann for the
    mask/K/gamma convention.
    ...
    a       = np.sqrt(K_loc)
    rho_lin = np.exp((R_plus - R_minus) / (2.0 * a))
    c_lin   = a

    with np.errstate(divide='ignore', invalid='ignore'):
        c_gen   = 0.25 * (gam_loc - 1.0) * (R_plus - R_minus)
        rho_gen = (c_gen**2 / (K_loc * gam_loc)) ** (1.0 / (gam_loc - 1.0))

    rho = np.where(is_linear, rho_lin, rho_gen)
    c   = np.where(is_linear, c_lin,   c_gen)
    return u, rho, c
...
class GhostCellConstructor:
    ...
    def build(self, fluid_id, i_Gamma, p_star, u_star,
              K_k, gamma_k, rho_real, u_real, p_real, n_override=None):
        """
        ...
        n_override         : if given, use this ghost width instead of
                              self.n_ghost -- lets a caller cap the fill
                              reach (e.g. when a second interface sits
                              closer than n_ghost cells away).

        Returns
        -------
        rho_ghost, u_ghost, p_ghost : (n_override or n_ghost,) arrays
        """
        n = self.n_ghost if n_override is None else n_override
        N = len(rho_real)
...
class MGFMSimulation:
    ...
    def __init__(self, x,
        ...
        # Ghost cell constructor
        self.ghosts = GhostCellConstructor(x, n_ghost, method='constant')
...
class MGFMSolver1F:
    """
    Two-fluid MGFM driver for a smooth IC that develops a shock, a material
    contact, or a mix of shock/contact/rarefaction interfaces.
    ...
    rho_0      : callable or (N,) array — initial density (shared: gives the
                 correct genuine density/velocity number on each side
                 regardless of which fluid's EOS interprets it there)
    u_0        : callable or (N,) array — initial velocity
    K, gamma   : fluid 1's isentropic EOS constants (solver1)
    K2, gamma2 : fluid 2's isentropic EOS constants (solver2). Default None
                 falls back to K, gamma -- i.e. the single-fluid degenerate
                 case exercised by every test so far. Pass distinct values
                 for a genuine two-fluid run.
    T_max      : final time
    cfl        : CFL number
    ...
    def __init__(self, x, rho_0, u_0,
                 ...
                 small_cell_fix=True,
                 disable_isolated_wave_ghost=False,
                 solver_kw=None):

        solver_kw = solver_kw or {}

        # Fluid 2 defaults to fluid 1's EOS if not given explicitly -- this
        # keeps every existing single-fluid call site (and its tested
        # degenerate behaviour) unchanged, while letting a caller pass
        # distinct (K2, gamma2) for a genuine two-fluid run.
        if K2 is None:
            K2 = K
        if gamma2 is None:
            gamma2 = gamma
        ...
        self.K2             = K2
        self.gamma2         = gamma2
        # (K1, K2) / (gamma1, gamma2) pairs consumed by to_riemann/from_riemann
        # via the fluid mask (0 -> fluid 1, 1 -> fluid 2).
        self._K_pair        = (K, K2)
        self._gamma_pair    = (gamma, gamma2)
        self.T_max          = T_max
        self.cfl            = cfl
        self.safety         = safety
        ...
        # Toggle: in a mixed wave/shock (or shock/wave) configuration, skip
        # the ghost injection anchored at the 'wave' side once it sits
        # farther than (2r+1) real cells from the paired 'shock' side --
        # i.e. once the two are isolated enough that the wave-side ghost
        # fill cannot be reached by anything the shock side is doing. Does
        # nothing in wave/wave or shock/shock configurations (no paired
        # shock to measure the distance against) and does nothing while
        # the two sides sit within (2r+1) cells of each other (kept exactly
        # as before). See step_post_shock_split for the gating logic.
        self.disable_isolated_wave_ghost = disable_isolated_wave_ghost
        ...
        self.solver2 = LPTSolver(x, rho_fn, u_fn, K=K2, gamma=gamma2, **solver_kw)
        self.tracker = ShockTracker(x)
        ...
        # HLLC reference on a finer grid (8× cells, same domain). This is a
        # single-EOS (fluid 1's, K/gamma) diagnostic comparator, meaningful
        # only for the single-fluid degenerate case (K2==K, gamma2==gamma)
        # -- there is no well-defined single-EOS HLLC reference for a
        # genuine two-fluid run (which fluid's EOS would it use across the
        # material contact?), so it is skipped entirely when the two EOS's
        # differ. Every call site that reads self.hllc must check
        # self.two_fluid / (self.hllc is not None) first.
        self.two_fluid = (K2 != K) or (gamma2 != gamma)
        if self.two_fluid:
            self.hllc = None
        else:
            self.hllc = HLLCSolver(x, rho_fn, u_fn, K=K, gamma=gamma, CFL=0.9)

        # Exact two-fluid interface Riemann solver: takes each side's own
        # EOS constants from self._K_pair/_gamma_pair.
        self.iface  = MGFMInterfaceSolver(self._K_pair[0], self._gamma_pair[0],
                                           self._K_pair[1], self._gamma_pair[1])
        self.ghosts = GhostCellConstructor(x, self.n_ghost, method=ghost_method)

        # Wave (rarefaction fan) tracker
        self.wave_tracker = WaveTracker(x)

        # Per-cell fluid mask (0 -> fluid 1, 1 -> fluid 2), kept live below.
        self.compute_fluid_mask()

    # ── fluid mask ───────────────────────────────────────────────────────────

    def compute_fluid_mask(self):
        """
        Per-cell fluid-identity mask (0 -> fluid 1, 1 -> fluid 2): which
        fluid is physically real at each grid cell for the current
        shock/interface configuration. Recomputed every step (called at
        the end of step_post_shock / step_post_shock_split) so it always
        reflects the latest tracked positions; stored on self.mask for any
        call site that needs a genuine per-cell mask instead of a
        hardcoded scalar, and used by plot_final for the fluid-1/fluid-2
        split shown in the final plot.

        Mixed case (tracker.interfaces = [left, contact, right]): cells <=
        the material contact's index are fluid 1, the rest are fluid 2.
        The contact -- not the L/R interfaces -- is the genuine
        fluid-identity boundary: the scratch strip either side of it is
        real physical material (rho_star_1 under fluid 1's own EOS on the
        left of the contact, rho_star_2 under fluid 2's own EOS on the
        right), whereas the L/R interfaces only bound the *scratch* region
        against each solver's untouched far-field domain.

        Shock/shock case (tracker.shocks): cells <= shocks[0]['i'] are
        fluid 1 (post-shock), the rest fluid 2 (pre-shock) -- only the
        leftmost tracked shock is used as the boundary, matching the
        existing plot_final convention.

        Pre-shock (neither tracked yet): everything is fluid 1.
        """
        N   = len(self.solver1.x)
        idx = np.arange(N)
        if self.tracker.interfaces:
            ifC  = next(f for f in self.tracker.interfaces if f['kind'] == 'contact')
            mask = (idx > ifC['i']).astype(int)
        elif self.tracker.shocks:
            i_bnd = self.tracker.shocks[0]['i']
            mask = (idx > i_bnd).astype(int)
        else:
    ...
    def detect_waves_from_rp(self, x_iface, t=0.0):
        ...
        K1, g1 = self._K_pair[0], self._gamma_pair[0]
        K2, g2 = self._K_pair[1], self._gamma_pair[1]

        rho_L = float(self.solver1.rho[i_L])
        u_L   = float(self.solver1.u[i_L])
        p_L   = float(pressure(rho_L, K1, g1))
        rho_R = float(self.solver2.rho[i_R])
        u_R   = float(self.solver2.u[i_R])
        p_R   = float(pressure(rho_R, K2, g2))

        p_star, u_star = self.iface.solve(rho_L, u_L, p_L, rho_R, u_R, p_R)
        # Step 3: real fluid-1 and fluid-2 material never mix, so each fan's
        # tail sits against its OWN plateau sub-state -- rho_star_1 under
        # fluid 1's EOS for the left/1-fan, rho_star_2 under fluid 2's EOS
        # for the right/2-fan. The two sub-states are separated by the
        # material contact moving at u_star.
        rho_star_1 = (p_star / K1) ** (1.0 / g1)
        rho_star_2 = (p_star / K2) ** (1.0 / g2)
        print(f"  [IRP]  u*={u_star:.6f}  p*={p_star:.6f}  "
              f"rho*_1={rho_star_1:.6f}  rho*_2={rho_star_2:.6f}")
        c_L      = float(sound_speed(rho_L,      K1, g1))
        c_R      = float(sound_speed(rho_R,      K2, g2))
        c_star_1 = float(sound_speed(rho_star_1, K1, g1))
        c_star_2 = float(sound_speed(rho_star_2, K2, g2))

        if p_star < p_L:   # 1-rarefaction (left-going fan)
            x_lo = x_iface + (u_L    - c_L     ) * t
            x_hi = x_iface + (u_star - c_star_1) * t
            # s_lo = head speed (u_L − c_L), s_hi = tail speed (u* − c*_1)
            self.wave_tracker.add_wave(x_lo, x_hi, family=1,
                                       s_lo=u_L    - c_L,
                                       s_hi=u_star - c_star_1)
            print(f"  [1-rarefaction] x_lo={x_lo:.4f}  x_hi={x_hi:.4f}  "
                  f"s_lo={u_L-c_L:.4f}  s_hi={u_star-c_star_1:.4f}")

        if p_star < p_R:   # 2-rarefaction (right-going fan)
            x_lo = x_iface + (u_star + c_star_2) * t
            x_hi = x_iface + (u_R    + c_R     ) * t
            # s_lo = tail speed (u* + c*_2), s_hi = head speed (u_R + c_R)
            self.wave_tracker.add_wave(x_lo, x_hi, family=2,
                                       s_lo=u_star + c_star_2,
                                       s_hi=u_R    + c_R)
            print(f"  [2-rarefaction] x_lo={x_lo:.4f}  x_hi={x_hi:.4f}  "
                  f"s_lo={u_star+c_star_2:.4f}  s_hi={u_R+c_R:.4f}")

    # ── interface classification ─────────────────────────────────────────────

    def init_interfaces_from_rp(self, x_iface=0.0, t=0.0):
        """
        Solve the interface Riemann problem once and set up the tracking
        structure the resulting wave configuration needs: always three
        interfaces bounding two EOS-consistent plateau sub-states (rho*_1,
        u*, p*) and (rho*_2, u*, p*), separated by the material contact
        (self.tracker.interfaces = [left, contact, right]):
          - a shock side uses the one-sided RH jump between its far state
            and its own plateau sub-state;
          - a wave side tracks ONLY the tail (the head is not tracked) at
            the constant characteristic speed u* -/+ c* (1-wave / 2-wave).

        This covers all four combinations uniformly -- wave/wave,
        wave/shock, shock/wave, and shock/shock -- since s_L/s_R below are
        already generic RH-or-characteristic formulas independent of the
        other side's kind, and step_post_shock_split's ghost injection /
        small-cell-fix / promoted-cell repair never special-case kind_L or
        kind_R beyond that per-side speed choice. (Earlier versions special-
        cased shock/shock into a single legacy RH-jump interface with no
        contact tracking at all -- self.tracker.shocks -- inherited from
        before the material contact existed as a concept. That path is
        strictly cruder than this one even for a single EOS on both sides
        genuinely double-shock Riemann problems still have an internal
        contact with rho*_1 != rho*_2 -- and for a genuine two-fluid setup
        with distinct EOS it was outright wrong: it collapsed three
        physically-distinct wave speeds into one averaged far-field RH
        speed and never tracked the contact. self.tracker.shocks is kept
        only for the *dynamically-detected* post-formation path
        (ShockTracker.locate_from_characteristics / add_new_shocks), not
        for this upfront IC classification anymore.)
        ...
        # Step 3: real fluid-1 and fluid-2 material never physically mix.
        # p_star, u_star are continuous/shared across the material contact,
        # but each side keeps its own isentropic density under its own EOS
        # -- so the "plateau" is really two distinct constant states,
        # rho_star_1 (fluid 1's own EOS) and rho_star_2 (fluid 2's own EOS),
        # separated by a third, self-similar interface moving at u_star.
        rho_star_1 = (p_star / K1) ** (1.0 / g1)
        c_star_1   = float(sound_speed(rho_star_1, K1, g1))
        rho_star_2 = (p_star / K2) ** (1.0 / g2)
        c_star_2   = float(sound_speed(rho_star_2, K2, g2))
        ...
        # interface for the whole run. p_star/u_star are shared across the
        # contact; rho_star_k/c_star_k are per-fluid.
        self.p_star, self.u_star       = p_star, u_star
        self.rho_star_1, self.c_star_1 = rho_star_1, c_star_1
        self.rho_star_2, self.c_star_2 = rho_star_2, c_star_2
        ...
        print(f"  [config] {kind_L}/contact/{kind_R} -> three interfaces "
              f"(u*={u_star:.6f}  p*={p_star:.6f}  "
              f"rho*_1={rho_star_1:.6f}  rho*_2={rho_star_2:.6f})")

        # kind_L is a jump strictly within fluid 1's own material (its
        # far state rho_L/u_L vs its own plateau sub-state rho_star_1)
        # -- same RH / characteristic-speed math as the old single-fluid
        # step_post_shock, just anchored on rho_star_1 instead of a
        # shared rho_star. Symmetric for kind_R with fluid 2. Note this is
        # a genuine one-sided RH jump even when BOTH sides are 'shock'
        # (shock/contact/shock): s_L only ever involves rho_L/u_L and
        # rho_star_1/u_star, never rho_R/u_R, so the two sides never
        # interfere with each other's speed regardless of kind.
        wave_l = (0 if abs(rho_star_1 - rho_L) < 1e-5 else 
                  (rho_star_1 * u_star - rho_L * u_L) / (rho_star_1 - rho_L))
        wave_r = (0 if abs(rho_star_2 - rho_R) < 1e-5 else 
                  (rho_R * u_R - rho_star_2 * u_star) / (rho_R - rho_star_2))
        
        s_L = (u_star - c_star_1 if kind_L == 'wave' else wave_l)
        s_R = (u_star + c_star_2 if kind_R == 'wave' else wave_r)

        print(f"    left    ({kind_L}):  x0={x_iface:.4f}  speed={s_L:.6f}")
        print(f"    contact         :  x0={x_iface:.4f}  speed={u_star:.6f}")
        print(f"    right   ({kind_R}):  x0={x_iface:.4f}  speed={s_R:.6f}")

        self.tracker.shocks = []
        self.tracker.interfaces = [
            # rho_far/u_far cache the genuine constant state on the far
            # (non-plateau) side of a 'shock'-kind interface -- needed by
            # step_post_shock_split's promoted-cell repair so that a cell
            # leaving the scratch region is stamped with the correct
            # physical state instead of a stale rho_star_k leftover. Not
            # meaningful for a 'wave'-kind side (the fan tail is
            # continuous with its own plateau sub-state there), nor for
            # the 'contact' (both its neighbors are already genuine
            # plateau sub-states, not "far" states) -- None in those cases.
            {'x': x_iface, 'i': i_s, 'kind': kind_L, 'side': 'L', 'speed': s_L,
             'rho_far': rho_L, 'u_far': u_L},
            {'x': x_iface, 'i': i_s, 'kind': 'contact', 'side': 'C', 'speed': u_star,
             'rho_far': None, 'u_far': None},
            {'x': x_iface, 'i': i_s, 'kind': kind_R, 'side': 'R', 'speed': s_R,
             'rho_far': rho_R, 'u_far': u_R},
        ]
    ...
    def _inject_wave_ghosts(self, solver, fluid_id=1):
        """
        ...
        fluid_id selects the fluid mask (0 -> fluid 1, 1 -> fluid 2) used
        to recover (u, rho) from R± via from_riemann.
        """
        if not self.wave_tracker.waves:
            return
        mask = 0 if fluid_id == 1 else 1
        n   = self.n_ghost
        N   = len(solver.R_plus)
        Rp  = solver.R_plus
        Rm  = solver.R_minus
        ...
        for wave in self.wave_tracker.waves:
            ...
            for sl in (g_lo, g_hi):
                ...
                u_g, rho_g, _ = from_riemann(Rp[sl].copy(), Rm[sl].copy(),
                                              mask, self._K_pair, self._gamma_pair)
                solver.u  [sl] = u_g
                solver.rho[sl] = rho_g

    def _inject_ghosts(self, fluid_id, rho_ghost, u_ghost, i_Gamma,
                       n_override=None, mask=None):
        """
        ...
        R± are recomputed from (rho_ghost, u_ghost) via to_riemann, using
        `mask` (0 -> fluid 1's EOS, 1 -> fluid 2's EOS) so the solver sees
        thermodynamically consistent values.

        `fluid_id` and `mask` are deliberately decoupled: fluid_id only
        picks which solver's array gets written and which direction
        convention the slice follows (this never changes -- solver1 always
        owns the array, solver2 always owns the other); mask picks which
        EOS the Riemann invariants are built from. Normally these coincide
        (mask defaults to fluid_id's own EOS below) -- that is the case for
        every ordinary ghost fill anchored at that fluid's own interface.
        The one exception is the material-contact crossover fill: solver1's
        array can still need filling *past* the contact with fluid 2's
        density (a real cell backed by a fluid-1 sub-strip narrower than
        the stencil radius must not sample the wrong fluid through that
        jump) -- there fluid_id=1 (still solver1's array, still its slice
        direction) but mask=1 (fluid 2's EOS) must be passed explicitly.

        n_override lets a caller shrink the fill width below n_ghost (must
        match whatever width was used to build rho_ghost/u_ghost).
        """
        n    = self.n_ghost if n_override is None else n_override
    ...
    def _compute_dt(self):
        ...
        dx     = self.solver1.dx
        lam1   = np.abs(self.solver1.u) + sound_speed(self.solver1.rho, self._K_pair[0], self._gamma_pair[0])
        lam2   = np.abs(self.solver2.u) + sound_speed(self.solver2.rho, self._K_pair[1], self._gamma_pair[1])
        dt_cfl = self.cfl * dx / max(float(np.max(lam1)), float(np.max(lam2)))
    ...
    def step_post_shock(self, dt):
        ...
        K1, g1 = self._K_pair[0], self._gamma_pair[0]
        K2, g2 = self._K_pair[1], self._gamma_pair[1]
        p_full1 = pressure(self.solver1.rho, K1, g1)
        p_full2 = pressure(self.solver2.rho, K2, g2)
        ...
        for shock in self.tracker.shocks:
            ...
            if 'rho_L' in shock:
                ...
                rho_R = float(self.solver2.rho[i_R])
                u_R   = float(self.solver2.u  [i_R])
            p_L   = pressure(rho_L, K1, g1)
            p_R   = pressure(rho_R, K2, g2)
        ...
        if self.minmax_limiter:
            for shock, (rho_L, u_L, rho_R, u_R, _, p_star, u_star) in zip(
                ...
                i      = shock['i']
                c_L    = sound_speed(rho_L, K1, g1)
                c_R    = sound_speed(rho_R, K2, g2)
                # Plateau: ambiguous/shared side for now, see detect_waves_from_rp.
                rho_st = (p_star / K1) ** (1.0 / g1)
                c_st   = sound_speed(rho_st, K1, g1)

                def _clamp(slv, fluid_id, sl, u_lo, u_hi, c_lo, c_hi):
                    mask     = 0 if fluid_id == 1 else 1
                    K_k, g_k = (K1, g1) if fluid_id == 1 else (K2, g2)
                    Rp = slv.R_plus [sl]
                    Rm = slv.R_minus[sl]
                    u_g, rho_g, c_g = from_riemann(Rp, Rm, mask,
                                                    self._K_pair, self._gamma_pair)
                    ...
                    if float(g_k) == 1.0:
                        ...
                        rho_g = (c_g**2 / (K_k * g_k)) ** (1.0 / (g_k - 1))
                    slv.u      [sl] = u_g
                    slv.rho    [sl] = rho_g
                    slv.R_plus [sl], slv.R_minus[sl] = to_riemann(
                        u_g, rho_g, mask, self._K_pair, self._gamma_pair)

                r       = self.solver1.r
                n_clamp = r 

                # Soft clamp inner real cells near interface
                _clamp(self.solver1, 1, slice(max(i+1-n_clamp, 0), i+1),
                       min(u_L, u_star), max(u_L, u_star),
                       min(c_L, c_st),   max(c_L, c_st))
                _clamp(self.solver2, 2, slice(i+1, min(i+1+n_clamp, N)),
        ...
        for shock, old_i, (rho_L, u_L, rho_R, u_R, *_) in zip(
            ...
            new_i = shock['i']
            if new_i > old_i:
                Rp, Rm = to_riemann(u_L, rho_L, 0, self._K_pair, self._gamma_pair)
                ...
                Rp, Rm = to_riemann(u_R, rho_R, 1, self._K_pair, self._gamma_pair)
    ...
    def _paired_interfaces(self):
        """
        Sort the tracked *domain-boundary* interfaces -- kind 'shock' or
        'wave', i.e. excluding the material 'contact' -- by grid index and
        yield each consecutive (left, right) couple together with:
          d      -- number of real grid nodes strictly inside the gap
                     (x_left, x_right);
          reach  -- ghost-fill width capped so neither side's fabricated
                    (plateau) ghost zone crosses into its neighbor's real
                    domain: reach = min(n_ghost, max(d, 0)).

        The 'contact' interface is excluded on purpose: it is an internal
        marker within the shared scratch/plateau region (it splits that
        region into two EOS-consistent sub-states), not a boundary between
        a solver's genuine real domain and the other solver's territory --
        the narrow-gap protection this generator provides is only about
        the latter, so it must key off the outer L/R distance regardless
        of where the contact currently sits between them.

        With the current wave/shock, shock/wave and wave/wave
        configurations there are always exactly two such boundary entries
        (L, R), so this loop runs once regardless of whether a 'contact'
        entry is also present in self.tracker.interfaces; it generalizes
        unchanged to a longer chain of boundary interfaces (e.g. a future
        multi-wave 2-fluid RP).
        """
        ifs = sorted((f for f in self.tracker.interfaces if f['kind'] != 'contact'),
                     key=lambda f: f['i'])
        n = self.n_ghost
        for left, right in zip(ifs[:-1], ifs[1:]):
            d     = right['i'] - left['i']
            reach = min(n, max(d, 0))
            yield left, right, d, reach

    def step_post_shock_split(self, dt):
        """
        One step for ANY interface RP configuration -- wave/wave, wave/shock,
        shock/wave, or shock/shock -- since kind_L/kind_R only ever affect
        the per-side speed formula (s_L/s_R, computed once in
        init_interfaces_from_rp) and whether promoted-cell repair applies to
        that side; nothing below special-cases the combination of kinds.
        Three interfaces (self.tracker.interfaces = [left, contact, right])
        bound two
        EOS-consistent plateau sub-states, (self.rho_star_1, self.u_star,
        self.p_star) and (self.rho_star_2, self.u_star, self.p_star),
        separated by the material contact.

        Each interface is self-similar (constant speed, cached in
        init_interfaces_from_rp), so there is nothing to re-solve here. All
        four possible ghost fills below go through the exact same two-step
        pipeline -- GhostCellConstructor.build() recovers a ghost
        (rho, u) segment from (p*, u*) plus whichever EOS is handed to it,
        then _inject_ghosts() writes it (and the matching R± invariants)
        into a solver's array -- the only thing that ever changes is which
        interface index the fill is anchored at and which EOS/mask it is
        given:
          - solver1 gets a ghost zone beyond the LEFT interface (anchored
            at i_L) built with its own EOS (K1, gamma1), giving rho_star_1;
          - solver2 gets a ghost zone beyond the RIGHT interface (anchored
            at i_R) built with its own EOS (K2, gamma2), giving rho_star_2;
          - u*, p* are continuous across the material contact ([u]=[p]=0)
            but rho is NOT -- a genuine physical jump -- so whenever the
            L or R ghost zone's reach would cross i_C, the remaining
            portion is instead built as its OWN ghost fill, re-anchored at
            i_C (same fluid_id / array / slice-direction convention as
            before, just a different anchor index) but under the OTHER
            fluid's EOS and mask. This is mathematically the same
            construction as a shock ghost fill: treating the contact "as a
            shock" degenerates correctly here because the general
            Rankine-Hugoniot speed s = (rho1*u1* - rho2*u2*)/(rho1*-rho2*)
            reduces to u* once u1*=u2*=u* is substituted in (the
            (rho1*-rho2*) factor cancels) -- so the only thing that
            actually differs between the two sides of a "shock-like"
            ghost fill at the contact is rho, recovered from the same p*
            via each side's own EOS, exactly like any other ghost fill;
          - the scratch strip between the two OUTER interfaces (if any
            cells wide) is pinned after the step, split at the contact
            index i_C: [i_L+1 : i_C+1] to (rho_star_1, u_star) and
            [i_C+1 : i_R+1] to (rho_star_2, u_star), since neither solver's
            SL stencil should be trusted there and the two sub-strips are
            genuinely different material.

        The contact DOES need its own repair, symmetric to the L/R
        demoted-cell fix: the plateau pin above uses i_L/i_C/i_R as they
        stood at the START of the step, but i_C itself can then move
        (either direction) during the advance below. Any cell swept from
        one sub-strip into the other by that motion was pinned to the
        WRONG fluid's rho_star_k just now and would sit one step stale
        otherwise -- e.g. a cell leaving the fluid-1 side and entering the
        fluid-2 side still shows rho_star_1 until repaired. This happens
        for contacts moving in either direction, not just one; it is
        fixed right after the interface advance, mirroring the L/R
        demoted-cell repair but keyed on i_C's own displacement.

        NOTE (ghost_method='linear' limitation): GhostCellConstructor.build
        in 'linear' mode blends (p_ghost, u_ghost) toward the true real
        boundary at the FAR end of a ghost zone (purely to avoid a Gibbs-
        inducing seam there -- not a physical jump). The crossover fills
        above call build() a second time anchored at i_C, which computes
        its OWN local "far boundary" reference from n_override=reach_b (or
        reach_d) alone -- that reference would land inside the still-
        fictional remainder of the scratch strip, not genuinely real data,
        breaking the blend's premise. This is inert in 'constant' mode
        (the only mode currently used -- p_ghost/u_ghost are flat
        regardless of the far-boundary reference) but would need
        build() split into a shared (p, u) blend computed once over the
        FULL combined reach, with rho recovered pointwise per-EOS on each
        side of i_C, before 'linear' mode could be safely combined with a
        live crossover. Left as a documented limitation until 'linear'
        mode is actually needed.

        Narrow-gap guard: if the two OUTER interfaces sit closer than
        n_ghost cells apart (down to zero real nodes between them, e.g.
        both inside the same grid cell), a solver's fabricated ghost zone
        would otherwise reach past the *other* outer interface into that
        solver's real domain -- corrupting it with a scratch value instead
        of the genuine far state. _paired_interfaces() (which ignores the
        contact -- see its own docstring) caps each side's ghost build at
        the neighboring outer interface; whatever width is left over
        (n_ghost - reach) is filled directly from the neighboring solver's
        own real values instead of being fabricated. This is a distinct
        concern from the rho switch-over above: that one protects against
        crossing the *contact* (an internal density jump inside the
        scratch region), this one protects against crossing the *other
        solver's real domain* boundary.
        """
        N = len(self.solver1.rho)
        n = self.n_ghost
        K1, g1 = self._K_pair[0], self._gamma_pair[0]
        ...
        r = self.solver1.r
        for left, right, d, reach in self._paired_interfaces():
            i_L, i_R = left['i'], right['i']

            # Isolated-wave toggle: only relevant to a genuine mixed pair
            # (one 'wave', one 'shock') sitting farther apart than the
            # (2r+1)-cell threshold. Skip the ghost fill anchored at
            # whichever side is the 'wave' one; the 'shock' side's fill
            # (and the narrow-gap borrow below, which cannot trigger here
            # since d > 2r+1 > n_ghost) is untouched either way.
            skip_left_ghost = skip_right_ghost = False
            if (self.disable_isolated_wave_ghost
                    and {left['kind'], right['kind']} == {'wave', 'shock'}
                    and d > 2 * r + 1):
                skip_left_ghost  = (left['kind']  == 'wave')
                skip_right_ghost = (right['kind'] == 'wave')

            if reach > 0:
                # solver1's ghost fill, anchored at i_L, reaching `reach`
                # cells rightward. u*, p* are continuous across the contact
                # but rho is NOT (genuine contact discontinuity) -- so the
                # fill must switch from rho_star_1 to rho_star_2 the moment
                # its reach would cross i_C, otherwise a real cell backed
                # by a fluid-1 sub-strip narrower than the stencil radius
                # would sample the wrong fluid's density straight through
                # that jump.
                if not skip_left_ghost:
                    reach_a = int(np.clip(i_C - i_L, 0, reach))   # still fluid-1 side
                    reach_b = reach - reach_a                     # crossed onto fluid-2 side
                    if reach_a > 0:
                        rho_g1, u_g1, _ = self.ghosts.build(
                            1, i_L, self.p_star, self.u_star, K1, g1,
                            self.solver1.rho, self.solver1.u, p_full1, n_override=reach_a)
                        self._inject_ghosts(1, rho_g1, u_g1, i_L, n_override=reach_a)
                    if reach_b > 0:
                        # Crossed past the contact: still solver1's array and
                        # still fluid_id=1's slice direction (anchored at i_C
                        # now instead of i_L), but recovered under fluid 2's
                        # EOS (K2, g2) and injected with mask=1 -- same
                        # build()/_inject_ghosts pipeline as every other ghost
                        # fill, just anchored at the contact instead of the
                        # solver's own outer interface.
                        rho_g1b, u_g1b, _ = self.ghosts.build(
                            1, i_C, self.p_star, self.u_star, K2, g2,
                            self.solver1.rho, self.solver1.u, p_full1, n_override=reach_b)
                        self._inject_ghosts(1, rho_g1b, u_g1b, i_C, n_override=reach_b, mask=1)

                # solver2's ghost fill, anchored at i_R, reaching `reach`
                # cells leftward -- symmetric split at the contact.
                if not skip_right_ghost:
                    reach_c = int(np.clip(i_R - i_C, 0, reach))   # still fluid-2 side
                    reach_d = reach - reach_c                     # crossed onto fluid-1 side
                    if reach_c > 0:
                        rho_g2, u_g2, _ = self.ghosts.build(
                            2, i_R, self.p_star, self.u_star, K2, g2,
                            self.solver2.rho, self.solver2.u, p_full2, n_override=reach_c)
                        self._inject_ghosts(2, rho_g2, u_g2, i_R, n_override=reach_c)
                    if reach_d > 0:
                        # Symmetric crossover: still solver2's array and
                        # fluid_id=2's slice direction, anchored at i_C, but
                        # recovered under fluid 1's EOS (K1, g1) and injected
                        # with mask=0.
                        rho_g2d, u_g2d, _ = self.ghosts.build(
                            2, i_C, self.p_star, self.u_star, K1, g1,
                            self.solver2.rho, self.solver2.u, p_full2, n_override=reach_d)
                        self._inject_ghosts(2, rho_g2d, u_g2d, i_C, n_override=reach_d, mask=0)

            if d < n:
                # Gap narrower than the stencil reach (down to d == 0, both
                # interfaces inside one cell): beyond the capped ghost zone,
                # borrow the neighboring solver's own real state rather than
                # fabricating a plateau value that would actually overlap
                # the other interface's real domain.
                lo1, hi1 = min(i_L + 1 + reach, N), min(i_L + 1 + n, N)
                if hi1 > lo1:
                    self.solver1.rho    [lo1:hi1] = self.solver2.rho    [lo1:hi1]
                    self.solver1.u      [lo1:hi1] = self.solver2.u      [lo1:hi1]
                    self.solver1.R_plus [lo1:hi1] = self.solver2.R_plus [lo1:hi1]
                    self.solver1.R_minus[lo1:hi1] = self.solver2.R_minus[lo1:hi1]

                lo2, hi2 = max(i_R + 1 - n, 0), max(i_R + 1 - reach, 0)
                if hi2 > lo2:
                    self.solver2.rho    [lo2:hi2] = self.solver1.rho    [lo2:hi2]
                    self.solver2.u      [lo2:hi2] = self.solver1.u      [lo2:hi2]
                    self.solver2.R_plus [lo2:hi2] = self.solver1.R_plus [lo2:hi2]
                    self.solver2.R_minus[lo2:hi2] = self.solver1.R_minus[lo2:hi2]

        ifL, ifC, ifR = self.tracker.interfaces
        i_L, i_C, i_R = ifL['i'], ifC['i'], ifR['i']
        ...
        # Pin the scratch strip to the exact star sub-states in both
        # solvers, split at the material contact: [i_L+1:i_C+1] is genuine
        # fluid-1 material (rho_star_1, u_star), [i_C+1:i_R+1] is genuine
        # fluid-2 material (rho_star_2, u_star).
        if i_C > i_L:
            Rp_1, Rm_1 = to_riemann(self.u_star, self.rho_star_1, 0, self._K_pair, self._gamma_pair)
            sl = slice(i_L + 1, i_C + 1)
            for slv in (self.solver1, self.solver2):
                slv.rho[sl]      = self.rho_star_1
                slv.u[sl]        = self.u_star
                slv.R_plus[sl]   = Rp_1
                slv.R_minus[sl]  = Rm_1
        if i_R > i_C:
            Rp_2, Rm_2 = to_riemann(self.u_star, self.rho_star_2, 1, self._K_pair, self._gamma_pair)
            sl = slice(i_C + 1, i_R + 1)
            for slv in (self.solver1, self.solver2):
                slv.rho[sl]      = self.rho_star_2
                slv.u[sl]        = self.u_star
                slv.R_plus[sl]   = Rp_2
                slv.R_minus[sl]  = Rm_2

        # Merge: solver1 owns [0..i_L], solver2 owns [i_R+1..N-1] -- unchanged
        # by the contact split, the scratch strip is already identical in
        # both after the two pins above.
        ...
        # Advance each interface, including the contact, at its own fixed
        # self-similar speed. i_L, i_C, i_R above are the pre-advance
        # ("old") indices.
        old_i_L, old_i_C, old_i_R = i_L, i_C, i_R
        for iface in (ifL, ifC, ifR):
            iface['x'] += iface['speed'] * dt
            iface['i']  = int(np.searchsorted(self.tracker.x, iface['x']) - 1)

        # Promoted-cell repair (only for 'shock'-kind OUTER sides): a cell
        # that just left the scratch region and entered a solver's genuine
        # real domain would otherwise retain the stale rho_star_k value
        # stamped by the pin above -- overwrite it with the cached genuine
        # far-field state instead.
        #
        # A 'wave'-kind side needs no repair: the fan tail is continuous
        # with its own plateau sub-state there. The contact never needs
        # repair either -- see the step_post_shock_split docstring.
        new_i_L, new_i_R = ifL['i'], ifR['i']
        if ifL['kind'] == 'shock' and new_i_L > old_i_L:
            rho_far, u_far = ifL['rho_far'], ifL['u_far']
            Rp_f, Rm_f = to_riemann(u_far, rho_far, 0, self._K_pair, self._gamma_pair)
            sl = slice(old_i_L + 1, new_i_L + 1)
            for slv in (self.solver1, self.solver2):
                slv.rho[sl]      = rho_far
                slv.u[sl]        = u_far
                slv.R_plus[sl]   = Rp_f
                slv.R_minus[sl]  = Rm_f
        if ifR['kind'] == 'shock' and new_i_R < old_i_R:
            rho_far, u_far = ifR['rho_far'], ifR['u_far']
            Rp_f, Rm_f = to_riemann(u_far, rho_far, 1, self._K_pair, self._gamma_pair)
            sl = slice(new_i_R + 1, old_i_R + 1)
            for slv in (self.solver1, self.solver2):
                slv.rho[sl]      = rho_far
                slv.u[sl]        = u_far
                slv.R_plus[sl]   = Rp_f
        ...
        # Demoted-cell repair (the mirror image, for the OTHER near node of
        # each shock-kind interface): when the interface instead retreats
        # -- widening the scratch strip by absorbing a genuine real cell --
        # that cell is NOT actually caught by the unconditional pin above,
        # despite the pin's "regardless of history" framing. The pin runs
        # BEFORE this advance, using the OLD (pre-advance) interface index,
        # so it only ever re-stamps the strip as it existed at the START of
        # this step. A cell about to be absorbed this step was still real
        # at pin-time -- worse, if it sat within small_cell_fix's reach of
        # the OLD interface position, small_cell_fix just overwrote it with
        # the genuine far-field value moments earlier in this same step.
        # Left uncorrected, it would sit one step stale at rho_far/u_far
        # instead of the plateau's rho_star_k/u_star, contaminating its
        # neighbours' stencils for a full step before next step's pin
        # (which will use the now-updated index) catches it -- the same
        # kind of transient contamination the promoted-cell repair above
        # already guards against, just for the opposite direction.
        if ifL['kind'] == 'shock' and new_i_L < old_i_L:
            Rp_s, Rm_s = to_riemann(self.u_star, self.rho_star_1, 0, self._K_pair, self._gamma_pair)
            sl = slice(new_i_L + 1, old_i_L + 1)
            for slv in (self.solver1, self.solver2):
                slv.rho[sl]      = self.rho_star_1
                slv.u[sl]        = self.u_star
                slv.R_plus[sl]   = Rp_s
                slv.R_minus[sl]  = Rm_s
        if ifR['kind'] == 'shock' and new_i_R > old_i_R:
            Rp_s, Rm_s = to_riemann(self.u_star, self.rho_star_2, 1, self._K_pair, self._gamma_pair)
            sl = slice(old_i_R + 1, new_i_R + 1)
            for slv in (self.solver1, self.solver2):
                slv.rho[sl]      = self.rho_star_2
                slv.u[sl]        = self.u_star
                slv.R_plus[sl]   = Rp_s
        ...
        # Contact-crossing repair: the docstring's claim that the contact
        # "never needs repair -- both neighbouring sub-strips are
        # unconditionally re-pinned every step regardless of where the
        # contact sits" turns out to suffer the exact same timing flaw as
        # the outer interfaces above. The pin ran BEFORE this advance,
        # using old_i_C: a cell that the contact's own motion sweeps from
        # one sub-strip into the other this step was pinned once, correctly
        # at pin-time, under its OLD side's state -- nothing revisits it
        # once the contact has actually moved past it. It sits one step
        # stale at the wrong fluid's (rho_star_k, u_star) until next step's
        # pin (now using the updated i_C) happens to overwrite it.
        # Reproduced directly by construction for both a left-going
        # ($u^*<0$) and a right-going ($u^*>0$) two-fluid contact -- the
        # stale cell shows up on whichever side the contact just vacated:
        # a stray rho_star_1 cell one past the new contact index for a
        # left-going contact, a stray rho_star_2 cell one before it for a
        # right-going one.
        new_i_C = ifC['i']
        if new_i_C > old_i_C:
            Rp_s, Rm_s = to_riemann(self.u_star, self.rho_star_1, 0, self._K_pair, self._gamma_pair)
            sl = slice(old_i_C + 1, new_i_C + 1)
            for slv in (self.solver1, self.solver2):
                slv.rho[sl]      = self.rho_star_1
                slv.u[sl]        = self.u_star
                slv.R_plus[sl]   = Rp_s
                slv.R_minus[sl]  = Rm_s
        elif new_i_C < old_i_C:
            Rp_s, Rm_s = to_riemann(self.u_star, self.rho_star_2, 1, self._K_pair, self._gamma_pair)
            sl = slice(new_i_C + 1, old_i_C + 1)
            for slv in (self.solver1, self.solver2):
                slv.rho[sl]      = self.rho_star_2
                slv.u[sl]        = self.u_star
                slv.R_plus[sl]   = Rp_s
    ...
    def plot_final(self, t):
        ...
        Mixed / wave-wave case: fluid 1 (left of the material contact) is
        blue, fluid 2 (right of the contact) is red -- color is driven
        purely by self.mask, so the scratch strip's two EOS-consistent
        sub-states are shown in their own fluid's color rather than a
        separate "plateau" color. Three dashed/dash-dot lines mark the
        left interface, the contact, and the right interface (L/R
        labelled by their kind, shock / wave).
        """
        ...
        # HLLC reference is only meaningful/available in the single-fluid
        # degenerate case (self.hllc is None for a genuine two-fluid run --
        # see the constructor docstring); fall back to no reference curve.
        if self.hllc is not None:
        ...
        if interfaces:
            ifL = next(f for f in interfaces if f['side'] == 'L')
            ifC = next(f for f in interfaces if f['kind'] == 'contact')
            ifR = next(f for f in interfaces if f['side'] == 'R')
            is_fluid1 = (self.mask == 0)
            is_fluid2 = (self.mask == 1)

            x_str = (f"L({ifL['kind']})={ifL['x']:.4f}  "
                     f"C(contact)={ifC['x']:.4f}  "
                     f"R({ifR['kind']})={ifR['x']:.4f}")
            color_str = "blue = fluid 1  |  red = fluid 2" if self.two_fluid else "blue = MGFM"
            fig.suptitle(
            ...
            for ax, y_mgfm, y_hllc, title in [
                ...
                if self.two_fluid:
                    ...
                    ax.scatter(x, y_mgfm,
                               color="blue",  marker="+", s=60, zorder=2, label="MGFM")
                ax.axvline(ifL['x'], color="k", linestyle="--", linewidth=1,
                           label=f"L ({ifL['kind']})")
                ax.axvline(ifC['x'], color="k", linestyle="-.", linewidth=1,
                           label="contact")
            ...
            # self.mask (0 -> fluid 1, 1 -> fluid 2) is kept live by
            # compute_fluid_mask() every step; fall back to the domain
            # midpoint split only if no shock has formed yet.
            if shocks:
                is_fluid1 = (self.mask == 0)
            else:
                i_bnd = N // 2 - 1
            ...
            color_str = ("blue = fluid 1 (post-shock)  |  red = fluid 2 (pre-shock)"
                         if self.two_fluid else "blue = MGFM")
            fig.suptitle(
                f"MGFM solution at t = {t:.4f}  —  "
                f"{color_str}  |  "
                f"x_Γ(RH) = [{x_gam_str}]"
...
# Fluid 1 (left of the material contact) and fluid 2 (right of it) are
# deliberately given distinct EOS constants below, so --direct_ic exercises
# a genuine two-fluid Riemann problem (rho*_1 != rho*_2 across the contact)
# out of the box, rather than the single-fluid degenerate case. Pass
# --K2/--gamma2 on the command line to override fluid 2's EOS instead.
_K_ST,  _GAM_ST  = 1., 1.4
_K2_ST, _GAM2_ST = 1., 1.1


''' old initial condition for 1-fluid
_S_CUSTOM   = .1
_RHO_L_CUSTOM = 4.0                 # 4.0
_U_L_CUSTOM   = _S_CUSTOM - .5     # = -0.45 with _S_CUSTOM - .5    
_RHO_R_CUSTOM = 1.0                 # 1.0
_U_R_CUSTOM   = -2. + _S_CUSTOM      # -2.0 + _S_CUSTOM
'''

# shock/contact/shock: both sides compressive (p* > p_L and p* > p_R), so
# --direct_ic now hits init_interfaces_from_rp's general 3-interface branch
# with kind_L='shock', kind_R='shock' -- verified via MGFMInterfaceSolver
# directly: p_L=5.6569, p_R=1.0, p*=6.5648 (> both) -> s_L=-0.8219 <
# u*=0.4404 < s_R=1.3679, rho*_1=4.5058 != rho*_2=3.0920 across the contact.

_S_CUSTOM = 1.01
_RHO_L_CUSTOM, _U_L_CUSTOM = 4., _S_CUSTOM + 1.0
_RHO_R_CUSTOM, _U_R_CUSTOM = 1., _S_CUSTOM - 2.5

# K1=1,γ1=1.4 ; K2=1,γ2=1.8 (defaults)

disable_ghost_wave_interface = False
cell_fix = False
...
if __name__ == "__main__":
    ...
    parser.add_argument("--K2",      type=float, default=None,
                        help="Fluid 2 EOS constant K. Defaults to --K (single-"
                             "fluid degenerate case) if not given.")
    parser.add_argument("--gamma2",  type=float, default=None,
                        help="Fluid 2 EOS exponent gamma. Defaults to --gamma "
                             "(single-fluid degenerate case) if not given.")
    ...
    if args.direct_ic:
        # Force fluid 1's EOS to match the hardcoded IC (linear EOS: p = K*rho, c = sqrt(K))
        if args.K != _K_ST or args.gamma != _GAM_ST:
            print(f"[direct IC]  overriding K={args.K}->{_K_ST}, gamma={args.gamma}->{_GAM_ST} "
                  f"to match IC EOS")
        args.K     = _K_ST
        args.gamma = _GAM_ST
        # Fluid 2 defaults to the deliberately-distinct (_K2_ST, _GAM2_ST) so
        # this is a genuine two-fluid run out of the box; an explicit
        # --K2/--gamma2 on the command line still overrides it.
        if args.K2 is not None and args.K2 != _K2_ST:
            print(f"[direct IC]  using user-specified K2={args.K2} "
                  f"(default would be {_K2_ST})")
        if args.gamma2 is not None and args.gamma2 != _GAM2_ST:
            print(f"[direct IC]  using user-specified gamma2={args.gamma2} "
                  f"(default would be {_GAM2_ST})")
        if args.K2 is None:
            args.K2 = _K2_ST
        if args.gamma2 is None:
            args.gamma2 = _GAM2_ST
        ...
        print(f"[direct IC]  fluid 1: K={_K_ST}  gamma={_GAM_ST}   "
              f"fluid 2: K2={args.K2}  gamma2={args.gamma2}   s={_S_CUSTOM}")
    ...
    # Fluid 2's EOS defaults to whatever fluid 1's ended up being (after any
    # of the branches above may have overridden args.K/args.gamma), i.e. the
    # single-fluid degenerate case, unless the user explicitly passed
    # --K2/--gamma2 for a genuine two-fluid run.
    if args.K2 is None:
        args.K2 = args.K
    if args.gamma2 is None:
        args.gamma2 = args.gamma
    ...
    sim = MGFMSolver1F(
        ...
        disable_isolated_wave_ghost = disable_ghost_wave_interface,
        solver_kw       = solver_kw,
    )
    ...
    if args.shock_tube:
        ...
        if sim.hllc is not None:
            ...
            print(f"  HLLC   : skipped (two-fluid run, K2/gamma2 != K/gamma)")
