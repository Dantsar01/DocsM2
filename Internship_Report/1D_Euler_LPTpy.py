...
class LPTSolver:
    ...
    def step(self, dt, interp_method='numpy'):
        ...
        # ── Strided RBF-GA derivatives (no NxN matrix) ───────────────────────
        # Sliding-window view of shape (N, M); pad with edge values (clamp BC).
        ...
        wins_Rp  = np.lib.stride_tricks.sliding_window_view(Rp_pad, M)  # (N, M)
        wins_Rm  = np.lib.stride_tricks.sliding_window_view(Rm_pad, M)  # (N, M)
        dR_plus  = wins_Rp @ w1                                     # (N,)
        dR_minus = wins_Rm @ w1                                     # (N,)
        
        Rp_pad   = np.pad(R_plus,  r + stencil_der_add, mode='edge')
        Rm_pad   = np.pad(R_minus, r + stencil_der_add, mode='edge')
        wins_Rp  = np.lib.stride_tricks.sliding_window_view(Rp_pad, M_der)  # (N, M)
        wins_Rm  = np.lib.stride_tricks.sliding_window_view(Rm_pad, M_der)  # (N, M)
        ddR_plus  = wins_Rp @ w2                                    # (N,)
        ddR_minus = wins_Rm @ w2                                    # (N,)
        ...
        # B± = -dA±/dt|_{λ±} = -(∂_t A± + λ± ∂_x A±)   [Lagrangian derivative of A±]
        B_p = -A_p_t - lam_p * A_p_x
        B_m = -A_m_t - lam_m * A_m_x
        ...
        if self.ck >= 1:
            R_plus  -= (A_p * dR_plus)  * (dt**2 / 2)
            R_minus -= (A_m * dR_minus) * (dt**2 / 2)
        if self.ck >= 2:
            R_plus  += (B_p * dR_plus)  * (dt**3 / 6)
            R_minus += (B_m * dR_minus) * (dt**3 / 6)
...
class EulerSimulation:
    ...
    def run(self, interp_method='numpy', kernel='se'):
        ...
        while t < self.T_max:
            ...
            dx = min(x[1:] - x[:-1])
            dt_ref = min(dx / max(abs((rho))), dx / max(abs((u)))) * self.cfl
